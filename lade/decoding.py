...
def greedy_search_proxy(self, *args, **kwargs):
    ...
    USE_DRAFT = int(os.environ.get("USE_DRAFT", 0))
...
def jacobi_greedy_search_multilevel_draft(
    self,
    input_ids: torch.LongTensor,
    logits_processor: Optional[LogitsProcessorList] = None,
    stopping_criteria: Optional[StoppingCriteriaList] = None,
    ...
    **model_kwargs,
    ...
    # split verified and draft here
    init_len = input_ids.shape[1] - (WINDOW_SIZE + LEVEL - 3)
    draft_tokens = input_ids[:, init_len:]
    ...
    # fill with draft here
    past_tokens = [draft_tokens[0].tolist()] + [None for _ in range(LEVEL - 2)]
    #past_tokens = [[set_token() for _ in range(WINDOW_SIZE + LEVEL - 3)]] + [None for _ in range(LEVEL - 2)]
